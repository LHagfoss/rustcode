...
plain commands, a reusable two-token prefix approval. Reusable approvals are
stored in `~/.config/rustcode/config.toml` as `approved_command_prefixes` and
match complete whitespace-delimited tokens. Shell operators, expansions,
redirections, network commands, privilege escalation, and known destructive
command families still require a fresh confirmation. RustCode does not isolate

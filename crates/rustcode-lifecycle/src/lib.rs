//! Dependency-free turn and stream lifecycle values.
//!
//! This crate deliberately has no UI, networking, configuration, or tool
//! execution dependencies. It owns stable lifecycle state and formatting;
//! application-specific orchestration remains in the root crate.
...
/// safe to retry without parsing log text.
...
pub fn stream_failure_kind_from_message(message: &str) -> Option<StreamFailureKind> {
    let name = message
...
/// Structured stream failure metadata. The counters distinguish a stream
/// that never emitted output from one that stopped partway through an event.
...
pub struct StreamFailure {
    ...
    pub partial_event_bytes: usize,
}

pub fn stop_reason_for_stream_failure(task_completed: bool, kind: StreamFailureKind) -> StopReason {
    if task_completed {
...
impl StreamFailure {
    pub fn new(kind: StreamFailureKind) -> Self {
        Self {
            kind,
            status: None,
            detail: None,
...
pub fn is_unavailable_tool_error(reason: &str) -> bool {
    reason.contains("unknown or unavailable tool")
}

pub fn final_transcript_content(
    task_completed: bool,
    content: &str,
